...
async function createRequest(user, data) {
  ...
  try {
    const row = await one(`
      ...
      [newPublicId('REQ'), user.id, user.team, callDate, callTime, phone,
       core.clean(data.callType, 100), hasCall ? 'new' : 'no_call', core.clean(data.comment, core.MAX_TEXT)]);
    return { success: true, requestId: row.public_id, status: row.status, message: 'Отправлено!' };
  } catch (e) {
...
function reqOut(r) {
  return {
    id: r.public_id,
    createdAt: r.created_at ? core.fmtDate(r.created_at) : '',
    ...
    checkedDate: r.checked_at ? core.fmtDate(r.checked_at) : '',
