...
async function staffByLogin(login) {
  return one(`SELECT id, full_name, team, role, login, position, password_hash, hired_at
...
// Уволенного оператора не вычёркиваем сразу: на его звонок ещё три месяца
// может прийти жалоба, и её надо разобрать. Войти он уже не может — логин
// освобождается при увольнении.
const KEEP_MONTHS = 3;
const STILL_VISIBLE =
  `(active OR (dismissed_at IS NOT NULL AND dismissed_at > current_date - interval '${KEEP_MONTHS} months'))`;

async function staffByName(name) {
  return one(`SELECT id, full_name, team, role FROM staff
               WHERE ${STILL_VISIBLE} AND full_name = $1
               ORDER BY active DESC LIMIT 1`, [String(name).trim()]);
}
async function operators(team) {
  // hired_at нужен производственным показателям: новичков со стажем меньше
  // месяца считают отдельной колонкой
  return team
    ? q(`SELECT id, full_name, team, hired_at, active, dismissed_at FROM staff
          WHERE ${STILL_VISIBLE} AND role='operator' AND team=$1 ORDER BY full_name`, [team])
    : q(`SELECT id, full_name, team, hired_at, active, dismissed_at FROM staff
          WHERE ${STILL_VISIBLE} AND role='operator' ORDER BY team, full_name`);
}
...
async function prepareEvaluation(payload) {
  ...
  // Контрольный звонок заказчика: обычная оценка, но её надо отличать —
  // в бланк для почты признак не попадает, там для него места нет.
  const controlCall = m.controlCall === true || m.controlCall === 'true';
  // Проект ДЦ: тот же чек-лист, но качество считают отдельно, а на
  // зарплату берут среднее ФСС и ДЦ вместе.
  const dc = m.dc === true || m.dc === 'true';
  ...
  return {
    ok: true, m, op, cfg, answers, comments, res, complaintSource, controlCall, dc,
    callDate: core.isoDate(cd.date),
...
async function saveEvaluation(user, payload) {
  ...
  if (!p.ok) return p;
  const { m, op, cfg, answers, comments, res, complaintSource, controlCall, dc,
      ...
      const ev = await t.one(`
        INSERT INTO evaluations
          (public_id, operator_id, qc_id, request_id, team, call_date, call_time, phone, iso_week,
           ...
           complaint_source, control_call, dc)
        ...
        [publicId, op.id, user.id, requestId, op.team, callDate, callTime, phone, week,
         ...
         res.violation, res.complaint, res.gratitude, res.complaintMark, complaintSource, controlCall, dc]);
...
async function getEvaluationCard(publicId) {
  ...
  return {
    ...
    meta: {
      ...
      controlCall: ev.control_call === true,
      dc: ev.dc === true,
      sentAt: ev.sent_at ? core.isoDate(ev.sent_at) : '',
...
async function myEvaluations(operatorId, limit, from, to) {
  const f = from ? core.isoDate(core.toDateObj(from)) : null;
  const t = to ? core.isoDate(core.toDateObj(to)) : null;
  const evs = await q(`
    SELECT e.public_id, e.call_date, e.call_time, e.phone, e.score, e.critical, e.minor,
           e.violation, e.complaint, e.gratitude, e.topic, e.control_call, e.dc,
           (e.seen_at IS NULL) AS is_new, q.full_name AS qc
      FROM evaluations e JOIN staff q ON q.id = e.qc_id
     WHERE e.operator_id = $1
       AND ($3::date IS NULL OR coalesce(e.sent_at, (e.created_at AT TIME ZONE 'Europe/Moscow')::date) >= $3)
       AND ($4::date IS NULL OR coalesce(e.sent_at, (e.created_at AT TIME ZONE 'Europe/Moscow')::date) <= $4)
     ORDER BY e.call_date DESC, e.id DESC LIMIT $2`, [operatorId, limit || 100, f, t]);
  ...
  return evs.map(e => ({
    ...
    topic: e.topic, qc: e.qc,
    // оператору важно видеть, что это был контрольный звонок заказчика
    controlCall: e.control_call === true,
    dc: e.dc === true,
    isNew: e.is_new === true,
...
// Оценки, которых оператор ещё не видел. Берём их отдельно от списка:
// список сужается фильтром по датам, а плашка при входе должна показать
// всё новое, даже если оно не попало в выбранный период.
async function unseenEvaluations(operatorId) {
  const rows = await q(`
    SELECT e.public_id, e.call_date, e.score, e.control_call, e.complaint,
           e.violation, e.gratitude, q.full_name AS qc
      FROM evaluations e JOIN staff q ON q.id = e.qc_id
     WHERE e.operator_id = $1 AND e.seen_at IS NULL
     ORDER BY e.control_call DESC, e.call_date DESC, e.id DESC`, [operatorId]);
  return rows.map(e => ({
    id: e.public_id,
    callDate: core.isoDate(e.call_date),
    score: Number(e.score), qc: e.qc,
    controlCall: e.control_call === true,
    complaint: e.complaint, violation: e.violation, gratitude: e.gratitude
  }));
}

// Оператор нажал «Понятно» — гасим все его новые разом.
async function markEvaluationsSeen(operatorId) {
  const rows = await q(`UPDATE evaluations SET seen_at = now()
     WHERE operator_id = $1 AND seen_at IS NULL RETURNING public_id`, [operatorId]);
  return rows.length;
}

module.exports = {
  getEvaluationCard, updateEvaluation, KEEP_MONTHS, STILL_VISIBLE,
  pool, q, one, tx,
  ...
  saveEvaluation, myEvaluations, unseenEvaluations, markEvaluationsSeen, newPublicId
};
