...
// От кого пришла жалоба — для сводки в отчёте по жалобам.
const COMPLAINT_SOURCES = ['Клиент', 'Заказчик'];

// Пункты-события, на которые завязаны отчёты по жалобам
const ITEM_COMPLAINT = 'B8P2';        // подтверждённая (обоснованная) жалоба
const ITEM_COMPLAINT_MARK = 'B8P3';   // признак жалобы в чек-листе
    ...
    b.items.forEach(function (it) {
      ...
      if (it.kind === 'flag') {
        if (a === V.YES) {
          ...
          if (it.rule === 'force0') {
            force0 = true;
            if (it.code === ITEM_COMPLAINT) complaint = true; else violation = true;
          }
...
module.exports = {
  ...
  COMPLAINT_SOURCES, ITEM_COMPLAINT, ITEM_COMPLAINT_MARK,
