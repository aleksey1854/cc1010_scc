...
async function getOperatorsList(token) {
  // Весь состав КЦ не должен быть виден рядовому оператору: интерфейс этот
  // вызов не использует, но точка входа открыта снаружи.
  const u = await auth.need(token, ['qc', 'sqc', 'rgo', 'srgo', 'manager', 'admin']);
  if (!u.success) return u;
  const ops = await db.operators(null);
  // уволенного держим в списке три месяца: на его звонок может прийти
  // жалоба. В выпадающем списке это должно быть видно сразу.
  return { success: true, operators: ops.map(o => ({
    fullName: o.full_name, group: o.team,
    dismissed: o.active === false ? core.fmtDate(o.dismissed_at) : ''
  })) };
}

async function getQcBootstrap(token) {
  // Форма оценки есть только у КК и админа. Оператору эта точка отвечала
  // и раньше — состав он уже не получал, но и звать её ему незачем.
...
// Правят в форме оценки, а она есть только у КК и админа: руководителю
// сервер разрешал, но зайти ему было некуда — держим списки одинаковыми.
const EDIT_ROLES = ['qc', 'sqc', 'admin'];
...
async function getMyEvaluations(token, from, to) {
  const u = await auth.resolveUser(token);
  if (!u.success) return u;
  const [mine, unseen] = await Promise.all([
    db.myEvaluations(u.id, 500, from, to),
    db.unseenEvaluations(u.id)
  ]);
  ...
  return {
    ...
    evaluations: mine.map(e => ({
      ...
      // контрольный звонок заказчика — оператор должен видеть, что его
      // слушал не только КЦ; без этого поля метка в кабинете не рисовалась
      controlCall: e.controlCall, isNew: e.isNew,
      failed: e.failed
    })),
    unseen: unseen.map(e => ({
      id: e.id, callDate: core.fmtDate(e.callDate) || e.callDate,
      score: e.score, qc: e.qc, controlCall: e.controlCall,
...
// Оператор прочитал плашку с новыми оценками — гасим её. Отдельной
// кнопкой, а не по факту открытия страницы: иначе уведомление мигнёт
// и пропадёт, пока человек ещё читает.
async function markEvaluationsSeen(token) {
  const u = await auth.resolveUser(token);
  if (!u.success) return u;
  const n = await db.markEvaluationsSeen(u.id);
...
// Диагностика. Без входа наружу уходит только «живо / не живо»:
// имя хоста Neon, название базы и численность состава — это подсказка
// тому, кто ищет, куда стучаться, а текст ошибки подключения может
// вынести наружу и строку соединения.
async function health(token) {
  let deep = false;
  if (token) {
    const u = await auth.resolveUser(token);
    deep = u.success && ['manager', 'admin'].indexOf(u.role) >= 0;
  }
  const raw = process.env.DATABASE_URL || '';
  let alive = false, err = '';
  try {
    await db.one('SELECT 1 AS c');
    alive = true;
  } catch (e) { err = e.message; }

  if (!deep) return { ok: alive, connect: alive ? 'ок' : 'ошибка' };
  ...
  out.connect = alive ? 'ок' : 'ошибка: ' + err;
  if (alive) out.staff = (await db.one('SELECT count(*)::int c FROM staff')).c;
  return out;
}
...
const HANDLERS = {
  ...
  saveEvaluation, getEvaluationCard, updateEvaluation, getMyEvaluations, markEvaluationsSeen,
  ...
  getDcReport: rep.getDcReport,
  getListeningPlan: rep.getListeningPlan,
  getMyUploadPlan: rep.getMyUploadPlan,
  getMyQuality: rep.getMyQuality,
  setSentDate: rep.setSentDate, deleteEvaluation: rep.deleteEvaluation,
  setListenMark: rep.setListenMark,
  ...
  setHiredDate: auth.setHiredDate, getMyTeamStaff: auth.getMyTeamStaff,
  // апелляции
  createAppeal: appeals.createAppeal, getAppeals: appeals.getAppeals,
  answerAppeal: appeals.answerAppeal, markAppealSeen: appeals.markAppealSeen,

  // сообщения о проблемах самого сайта
  createBugReport: bugs.createBugReport, getBugReports: bugs.getBugReports,
  answerBugReport: bugs.answerBugReport
};
