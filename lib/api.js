...
// это не лист. Интерфейс подставляет их в «Критерий звонка».
const CALL_CRITERIA = [
...
async function getQcBootstrap(token) {
  ...
  const [cfg, ops] = await Promise.all([getChecklistConfig(), getOperatorsList(token)]);
...
const HANDLERS = {
  login, logoutSession, changePassword,
  getChecklistConfig, getOperatorsList, getQcBootstrap,
  createRequest, updateRequest, deleteRequest, reviewRequest,
  getOperatorRequests, getOperatorStats, getRequestsByOperator, getAllRequests,
  saveEvaluation, getMyEvaluations, getOperatorBootstrap,
