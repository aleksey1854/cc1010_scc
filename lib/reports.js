...
async function getProductionReport(token, fromArg, toArg, group) {
  ...
  const ensure = (name, tm) => {
    if (!byOp.has(name)) byOp.set(name, { name, team: tm || '', scores: [], pj: [], trainee: false });
  ...
  evs.forEach(e => {
    ...
    (e.is_pj ? o.pj : o.scores).push({
      id: e.public_id, score: core.round2(e.score), date: core.fmtDate(e.call_date),
  ...
  for (const o of byOp.values()) {
    ...
    groups.get(g).operators.push({
      name: o.name, avg: avg(o.scores), scores: o.scores, pj: o.pj,
      trainee: !!o.trainee,
...
async function getMyQuality(token, fromArg, toArg) {
  ...
  const r = await db.one(`
    SELECT count(*) FILTER (WHERE NOT (complaint OR complaint_mark))::int        AS n,
     ...
     WHERE operator_id = $1
       AND ($2::date IS NULL OR call_date >= $2)
       AND ($3::date IS NULL OR call_date <= $3)`, [user.id, from, to]);
...
async function exportReport(token, kind, params) {
  ...
  if (kind === 'production') {
    ...
    const cols = [
      ...
      { header: 'Жалоб необоснованных', key: 'pju', align: 'center' }
    ];
    ...
    r.groups.forEach(g => {
      g.operators.forEach(o => {
        const row = { group: g.name, operator: o.name, n: o.scores.length,
                      pjc: o.pjConfirmed, pju: o.pjUnconfirmed, senior: '',
