...
async function prepareEvaluation(payload) {
  ...
  return {
    ok: true, m, op, cfg, answers, comments, res, complaintSource, controlCall,
    callDate: core.isoDate(cd.date),
...
async function saveEvaluation(user, payload) {
  ...
  if (!p.ok) return p;
  const { m, op, cfg, answers, comments, res, complaintSource, controlCall,
    ...
    return await tx(async (t) => {
      ...
      const ev = await t.one(`
        ...
        [publicId, op.id, user.id, requestId, op.team, callDate, callTime, phone, week,
         ...
         res.violation, res.complaint, res.gratitude, res.complaintMark, complaintSource, controlCall]);
...
async function myEvaluations(operatorId, limit, from, to) {
  ...
  const evs = await q(`
    SELECT e.public_id, e.call_date, e.call_time, e.phone, e.score, e.critical, e.minor,
           e.violation, e.complaint, e.gratitude, e.topic, e.control_call,
