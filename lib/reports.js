...
async function exportReport(token, kind, params) {
  ...
  if (kind === 'journal') {
    ...
    return xlsx.sheet('journal', 'Журнал оценок', [
      ...
      id: x.id, date: x.callDate, time: x.callTime || '', operator: x.operator, group: x.group,
  ...
  if (kind === 'complaints') {
    ...
    return xlsx.sheet('zhaloby', 'Жалобы', [
      ...
      { header: 'Всего', key: 'total', align: 'center' },
      { header: 'Обоснованные', key: 'confirmed', align: 'center' },
      { header: 'Необоснованные', key: 'unconfirmed', align: 'center' },
      { header: 'Доля обоснованных', key: 'share', numFmt: PCT, align: 'center' }
    ], r.rows.map(x => ({
      topic: x.topic, week: x.week, total: x.total,
      confirmed: x.confirmed, unconfirmed: x.unconfirmed, share: Number(x.share)
    })));
  }
  ...
  if (kind === 'checklists' || kind === 'evaluation') {
    const one = kind === 'evaluation';
    let list;
    if (one) {
      const j = await getJournal(token, { period: 'all' });
      if (!j.success) return j;
      list = j.rows.filter(r => r.id === String(p.id || ''));
      if (!list.length) return { success: false, error: 'Оценка не найдена: ' + (p.id || '—') };
    } else {
