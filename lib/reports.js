...
async function getKkReport(token, from, to) {
  ...
  const rows = await db.q(`
    SELECT s.full_name, s.team, s.hired_at,
           count(e.id) FILTER (WHERE NOT e.dc AND ${RD('e')} >= $2 AND ($3::date IS NULL OR ${RD('e')} <= $3))::int AS n,
           round(avg(e.score) FILTER (WHERE NOT e.dc AND ${RD('e')} >= $2 AND ($3::date IS NULL OR ${RD('e')} <= $3)), 2)::float AS avg,
           count(*) FILTER (WHERE e.violation AND ${RD('e')} >= $2 AND ($3::date IS NULL OR ${RD('e')} <= $3))::int AS ko,
           count(*) FILTER (WHERE e.complaint AND e.complaint_source = 'Заказчик'
                              AND ${RD('e')} >= $2 AND ($3::date IS NULL OR ${RD('e')} <= $3))::int AS pj_zak,
           count(*) FILTER (WHERE e.complaint AND e.complaint_source <> 'Заказчик'
                              AND ${RD('e')} >= $2 AND ($3::date IS NULL OR ${RD('e')} <= $3))::int AS pj_kli
     ...
     ORDER BY s.team, s.full_name`, [team, f, t]);
