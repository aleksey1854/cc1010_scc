...
const KZ_FILL = 'FFF1E9FF';   // фиолетовая заливка колонок КЗ, как на экране
...
async function getJournal(token, params) {
  ...
  const rows = await db.q(`
    SELECT e.id, e.public_id, e.call_date, e.call_time, e.phone, e.score, e.critical, e.minor,
           ...
           e.created_at, e.sent_at, e.control_call, e.dc, ${RD('e')} AS rep_date
      FROM evaluations e
     ...
     WHERE ($1::text IS NULL OR e.team = $1)
       ...
       AND ($12::bool IS NOT TRUE OR e.dc)
     ORDER BY e.call_date DESC, e.id ASC LIMIT 1000`,
    [team, from, yes(p.onlyErrors), yes(p.onlyComplaint), to, opName, qcName,
     yes(p.onlyViolation), phone, yes(p.onlyAnyComplaint), yes(p.onlyControl), yes(p.onlyDc)]);
...
// Отчёт КК за выбранный период. Раньше тут были две жёсткие колонки —
// «за неделю» и «за месяц» от сегодняшней даты; теперь период выбирают
// руками, а недельный срез получается выставленной неделей.
//
// Жалобы разложены по источнику: заказчик и клиент считаются отдельно,
// потому что спрашивают за них по-разному.
async function getKkReport(token, from, to) {
  const user = await auth.resolveUser(token);
  if (!user.success) return user;
  if (!auth.canReportScoped(user.role)) return { success: false, error: 'Нет доступа' };
  const team = auth.teamFilter(user);

  // Пустой или неразобранный период — берём текущую неделю: отчёт
  // сдают по понедельникам, и это самый частый срез.
  const fd = from ? core.toDateObj(from) : null;
  const td = to ? core.toDateObj(to) : null;
  const f = fd ? core.isoDate(fd) : core.isoDate(core.periodStart('week'));
  const t = td ? core.isoDate(td) : null;

  // Оценки считаем по группе, которая записана в самой оценке: так
  // переведённый посреди недели оператор стоит двумя строками, старая
  // группа со своими цифрами и новая со своими. Состав берём отдельно —
  // он даёт нулевые строки тем, кого за период не слушали.
  const [roster, stats] = await Promise.all([
    db.q(`SELECT full_name, team, hired_at FROM staff
           WHERE active AND role = 'operator' AND ($1::text IS NULL OR team = $1)`, [team]),
    db.q(`
    SELECT s.full_name, coalesce(nullif(e.team, ''), s.team) AS team, s.hired_at,
           count(*)::int AS n,
           round(avg(e.score), 2)::float AS avg,
           count(*) FILTER (WHERE e.violation)::int AS ko,
           count(*) FILTER (WHERE e.complaint AND e.complaint_source = 'Заказчик')::int AS pj_zak,
           count(*) FILTER (WHERE e.complaint AND e.complaint_source <> 'Заказчик')::int AS pj_kli
      FROM evaluations e JOIN staff s ON s.id = e.operator_id
     WHERE s.active AND s.role = 'operator'
       -- отчёт КК — это ФСС. По ДЦ собирают только плановую прослушку
       -- (Даниил, 23.09), КО и жалобы по нему не сдаются вовсе, а раньше
       -- у оператора могло выйти «0 оценок, 1 КО»
       AND NOT e.dc
       AND ($1::text IS NULL OR e.team = $1)
       AND ${RD('e')} >= $2 AND ($3::date IS NULL OR ${RD('e')} <= $3)
     GROUP BY 1, 2, 3`, [team, f, t])
  ]);

  const byKey = new Map();
  const key = (name, tm) => name + '\u0000' + (tm || '');
  stats.forEach(r => byKey.set(key(r.full_name, r.team), r));
  roster.forEach(r => {
    if (!byKey.has(key(r.full_name, r.team))) {
      byKey.set(key(r.full_name, r.team),
        { full_name: r.full_name, team: r.team, hired_at: r.hired_at, n: 0, avg: null, ko: 0, pj_zak: 0, pj_kli: 0 });
    }
  });
  const rows = [...byKey.values()].sort((a, b) =>
    (a.team || '').localeCompare(b.team || '', 'ru') || a.full_name.localeCompare(b.full_name, 'ru'));

  const now = new Date();
  return {
    success: true,
    from: core.fmtDate(f), to: t ? core.fmtDate(t) : '',
    rows: rows.map(r => ({
      operator: r.full_name, group: r.team,
      months: r.hired_at ? core.monthsBetween(r.hired_at, now) : '',
      count: r.n, avg: r.n ? r.avg : null,
      ko: r.ko,
      pjCustomer: r.pj_zak, pjClient: r.pj_kli,
      complaints: r.pj_zak + r.pj_kli
    }))
  };
}
...
async function getProductionReport(token, fromArg, toArg, group) {
  ...
  const [ops, evs] = await Promise.all([
    db.operators(team),
    db.q(`
      SELECT s.full_name AS op, coalesce(nullif(e.team, ''), s.team) AS team, s.hired_at,
             e.score::float AS score, e.public_id,
       ...
       WHERE ($1::text IS NULL OR e.team = $1)
         ...
         -- ДЦ живёт своим отчётом: в качестве ФСС ему делать нечего
         AND NOT e.dc
       ORDER BY 2, s.full_name, rep_date, e.id`, [team, from, to])
  ]);

  // Строка отчёта — это оператор в группе, а не просто оператор. Оценка
  // помнит группу на момент проверки, и если человека перевели посреди
  // периода (ПТП расформировали 01.10), он стоит двумя строками: старая
  // группа со своими оценками и новая со своими. Иначе чужие оценки
  // уехали бы в новую группу задним числом.
  const byOp = new Map();
  ...
  const ensure = (name, tm, hired) => {
    const key = name + '\u0000' + (tm || '');
    if (!byOp.has(key)) {
      byOp.set(key, { name, team: tm || '', scores: [], pj: [], kz: [],
                      hiredAt: hired || null,
                      trainee: !!hired && core.monthsBetween(hired, asOf) < 1 });
    }
    return byOp.get(key);
  };
  // в отчёте должны стоять все, включая непрослушанных: пустая строка —
  // это тоже показатель
  ops.forEach(o => ensure(o.full_name, o.team, o.hired_at));
  evs.forEach(e => {
    const o = ensure(e.op, e.team, e.hired_at);
    const item = {
  ...
  // Колонку КЗ держим всегда, даже когда контрольных звонков не было:
  // это фиксированное место в отчёте, а не то, что появляется и пропадает.
  const maxKz = Math.max(1, ...list.map(g => Math.max(0, ...g.operators.map(o => o.kz.length))));
  ...
  return {
    ...
    // переведённый стоит в двух группах, но человек он один
    operators: new Set([...byOp.values()].map(o => o.name)).size,
...
// ---------- ПРОЕКТ ДЦ ----------
// Универсалы работают и на ФСС, и на ДЦ. Качество по ДЦ считают
// отдельно, а на зарплату берут среднее по обоим проектам — поэтому
// в одной таблице и ФСС, и ДЦ, и общее среднее.
async function getDcReport(token, fromArg, toArg, group) {
  const user = await auth.resolveUser(token);
  if (!user.success) return user;
  if (!auth.canReportScoped(user.role)) return { success: false, error: 'Нет доступа' };
  ...
  const rows = await db.q(`
    SELECT s.full_name AS op, s.team, e.score::float AS score, e.public_id, e.dc,
           ${RD('e')} AS rep_date
      FROM evaluations e JOIN staff s ON s.id = e.operator_id
     WHERE ($1::text IS NULL OR e.team = $1)
       AND ($2::date IS NULL OR ${RD('e')} >= $2)
       AND ($3::date IS NULL OR ${RD('e')} <= $3)
       -- неподтверждённые жалобы никуда не считаются, как и везде
       AND NOT (e.complaint_mark AND NOT e.complaint)
     ORDER BY s.team, s.full_name, 6`, [team, from, to]);

  const byOp = new Map();
  for (const r of rows) {
    if (!byOp.has(r.op)) byOp.set(r.op, { name: r.op, team: r.team, dc: [], fss: [] });
    const o = byOp.get(r.op);
    (r.dc ? o.dc : o.fss).push({ id: r.public_id, score: core.round2(r.score), date: core.fmtDate(r.rep_date) });
  }

  const avg = a => a.length ? core.round2(a.reduce((s, x) => s + x.score, 0) / a.length) : null;

  // в таблицу попадают только те, у кого ДЦ вообще был
  const list = [...byOp.values()].filter(o => o.dc.length)
    .sort((a, b) => (a.team || '').localeCompare(b.team || '', 'ru') || a.name.localeCompare(b.name, 'ru'))
    .map(o => ({
      operator: o.name, group: o.team,
      dcCount: o.dc.length, dcAvg: avg(o.dc), dcScores: o.dc,
      fssCount: o.fss.length, fssAvg: avg(o.fss),
      bothCount: o.dc.length + o.fss.length,
      bothAvg: avg(o.dc.concat(o.fss))
    }));

  const всеДЦ = [].concat(...[...byOp.values()].map(o => o.dc));
  const всеФСС = [].concat(...[...byOp.values()].filter(o => o.dc.length).map(o => o.fss));

  return {
    success: true,
    from: core.fmtDate(from), to: to ? core.fmtDate(to) : '',
    dcAvg: avg(всеДЦ), dcCount: всеДЦ.length,
    bothAvg: avg(всеДЦ.concat(всеФСС)), bothCount: всеДЦ.length + всеФСС.length,
    operators: list.length,
    maxDc: Math.max(1, ...list.map(o => o.dcCount)),
    rows: list
  };
}
...
async function exportReport(token, kind, params) {
  ...
  if (kind === 'dc') {
    const r = await getDcReport(token, p.from, p.to, p.group);
    if (!r.success) return r;
    const cols = [
      { header: 'Группа', key: 'group' },
      { header: 'Оператор', key: 'operator', width: 30 },
      { header: 'Оценок ДЦ', key: 'dcCount', align: 'center' },
      { header: 'Качество ДЦ', key: 'dcAvg', numFmt: PCT, align: 'center' },
      { header: 'Оценок ФСС', key: 'fssCount', align: 'center' },
      { header: 'Качество ФСС', key: 'fssAvg', numFmt: PCT, align: 'center' },
      { header: 'Среднее ФСС + ДЦ', key: 'bothAvg', numFmt: PCT, align: 'center', width: 18 }
    ];
    for (let i = 0; i < r.maxDc; i++) {
      cols.push({ header: 'ДЦ ' + (i + 1), key: 'd' + i, numFmt: PCT, align: 'center' });
    }
    const rows = r.rows.map(o => {
      const row = { group: o.group, operator: o.operator,
                    dcCount: o.dcCount, dcAvg: o.dcAvg === null ? '' : o.dcAvg,
                    fssCount: o.fssCount, fssAvg: o.fssAvg === null ? '' : o.fssAvg,
                    bothAvg: o.bothAvg === null ? '' : o.bothAvg };
      o.dcScores.forEach((x, i) => { row['d' + i] = x.score; });
      return row;
    });
    rows.push({ group: '', operator: 'Итого по ДЦ', dcCount: r.dcCount,
                dcAvg: r.dcAvg === null ? '' : r.dcAvg, bothAvg: r.bothAvg === null ? '' : r.bothAvg,
                __total: true });
    return xlsx.sheet('dc', 'Проект ДЦ', cols, rows);
  }

  if (kind === 'kk') {
    const r = await getKkReport(token, p.from, p.to);
    if (!r.success) return r;
    return xlsx.sheet('otchet-kk', 'Отчёт КК', [
      { header: 'Оператор', key: 'operator' },
      { header: 'Группа', key: 'group' },
      { header: 'Стаж, мес', key: 'months', align: 'center' },
      { header: 'Оценок за период', key: 'count', align: 'center' },
      { header: '% без КО за период', key: 'avg', numFmt: PCT, align: 'center' },
      { header: 'Недопустимых событий', key: 'ko', align: 'center' },
      { header: 'Жалоб от заказчика', key: 'pjCustomer', align: 'center' },
      { header: 'Жалоб от клиента', key: 'pjClient', align: 'center' }
    ], r.rows.map(x => ({
  ...
  if (kind === 'production') {
    ...
    const cols = [
      ...
      { header: 'КЗ от заказчика', key: 'kz', align: 'center', fill: KZ_FILL }
    ];
    for (let i = 0; i < r.maxScores; i++) {
      cols.push({ header: 'Оценка ' + (i + 1), key: 's' + i, numFmt: PCT, align: 'center' });
    }
    for (let i = 0; i < r.maxKz; i++) {
      cols.push({ header: 'КЗ ' + (i + 1), key: 'k' + i, numFmt: PCT, align: 'center', fill: KZ_FILL });
    }
...
module.exports = {
  getRgoDashboard, getOrgDashboard, getJournal, getKkReport,
  getCriteriaReport, getTopicsReport, getComplaintsReport, getProductionReport, getDcReport,
