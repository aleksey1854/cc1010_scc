...
async function operators(team) {
  ...
  return team
    ? q(`SELECT id, full_name, team, hired_at FROM staff WHERE active AND role='operator' AND team=$1 ORDER BY full_name`, [team])
    : q(`SELECT id, full_name, team, hired_at FROM staff WHERE active AND role='operator' ORDER BY team, full_name`);
}
