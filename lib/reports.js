...
async function exportReport(token, kind, params) {
  ...
  if (kind === 'checklists') {
    ...
    const pubs = j.rows.map(r => r.id);
    const nums = await db.q(`SELECT id, public_id FROM evaluations WHERE public_id = ANY($1)`, [pubs]);
    ...
    for (const ev of j.rows) {
      ...
      for (const b of cfg.blocks) {
        for (const it of b.items) {
          ...
          rows.push([
            ev.id, ev.callDate, ev.callTime || '', ev.operator, ev.group, ev.qc,
            ev.score, ev.topic || '', b.name, it.text, val, a ? (a.comment || '') : ''
          ]);
        }
      }
    }
    return {
      success: true,
      filename: 'checklists-' + j.rows.length + '.csv',
      content: toCsv(
        ['Оценка', 'Дата', 'Время, МСК', 'Оператор', 'Группа', 'Контролёр', 'Итог %', 'Тематика',
         'Блок', 'Пункт', 'Результат', 'Комментарий'], rows)
    };
  }

  // Один чек-лист целиком: метаданные звонка и каждый пункт с ответом.
  // Кнопка выгрузки в журнале звала этот вид, а его не существовало.
  if (kind === 'evaluation') {
    const ev = await db.one(`
      SELECT e.*, s.full_name AS operator, q.full_name AS qc
        FROM evaluations e
        JOIN staff s ON s.id = e.operator_id
        JOIN staff q ON q.id = e.qc_id
       WHERE e.public_id = $1`, [String(p.id || '')]);
    if (!ev) return { success: false, error: 'Оценка не найдена: ' + (p.id || '—') };

    const [given, cfg] = await Promise.all([
      db.q(`SELECT item_code, value::text AS value, comment
              FROM evaluation_answers WHERE evaluation_id = $1`, [ev.id]),
      db.getChecklist()
    ]);
    const byCode = new Map(given.map(a => [a.item_code, a]));

    const rows = [
      ['Оценка', ev.public_id, '', ''],
      ['Оператор', ev.operator, '', ''],
      ['Группа', ev.team || '', '', ''],
      ['Контролёр', ev.qc, '', ''],
      ['Дата звонка', core.fmtDate(ev.call_date), '', ''],
      ['Время звонка, МСК', ev.call_time || '', '', ''],
      ['Телефон', ev.phone || '', '', ''],
      ['Тематика', [ev.topic, ev.subtopic].filter(Boolean).join(' / '), '', ''],
      ['Город', [ev.city, ev.agglomeration].filter(Boolean).join(' / '), '', ''],
      ['Критерий', ev.criterion || '', '', ''],
      ['Итог', core.pct2(Number(ev.score)), '', ''],
      ['Критичных / некритичных', ev.critical + ' / ' + ev.minor, '', ''],
      ['', '', '', '']
    ];
    for (const b of cfg.blocks) {
      for (const it of b.items) {
        const a = byCode.get(it.code);
        // храним только отклонения: чего нет — то «Положительно», у событий «Не обнаружено»
        const val = a ? (core.CODE_TO_RU[a.value] || a.value)
                      : (it.kind === 'score' ? core.CODE_TO_RU.pos : core.CODE_TO_RU.no);
        rows.push([b.name, it.text, val, a ? (a.comment || '') : '']);
      }
    }
    return { success: true, filename: 'checklist-' + ev.public_id + '.csv',
             content: toCsv(['Блок', 'Пункт', 'Результат', 'Комментарий'], rows) };
  }
