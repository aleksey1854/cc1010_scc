...
async function login(loginName, password) {
  ...
  if (okPass) {
    ...
    return {
      ...
      // должность нужна шапке сразу после входа, а не только при обновлении
...
async function resolveUser(token) {
  ...
  const row = await db.one(`
    SELECT s.id, s.full_name, s.team, s.role, s.login, s.position
...
async function addUser(token, fullName, group, role, login, password, hiredAt, trainingAt, position) {
  const u = await need(token, STAFF_ADMINS);
  if (!u.success) return u;
  ...
  try {
    await db.q(`INSERT INTO staff (full_name, team, role, login, password_hash, hired_at, training_at, position)
      ...
      [String(fullName).trim(), String(group || '').trim(), String(role).trim(), lg,
       ...
       String(position || '').trim().slice(0, 120)]);
...
async function updateUser(token, oldFullName, fullName, group, role, hiredAt, trainingAt, position) {
  const u = await need(token, STAFF_ADMINS);
  if (!u.success) return u;
  if (core.ALLOWED_ROLES.indexOf(String(role).trim()) < 0) return { success: false, error: 'Недопустимая роль: ' + role };
  ...
  const r = await db.one(`UPDATE staff SET full_name=$2, team=$3, role=$4,
    ...
    [String(oldFullName).trim(), String(fullName).trim(), String(group || '').trim(), String(role).trim(),
     asDate(hiredAt), asDate(trainingAt), String(position || '').trim().slice(0, 120)]);
...
async function deleteUser(token, fullName) {
  ...
  // Мягкое увольнение: оценки остаются в отчётах, а сам человек ещё три
  // месяца виден в списках — на его звонок может прийти жалоба.
  const r = await db.one(`UPDATE staff SET active=false, login=NULL,
                                 dismissed_at=current_date, updated_at=now()
                           WHERE active AND full_name=$1 RETURNING id`, [String(fullName).trim()]);
  if (!r) return { success: false, error: 'Пользователь не найден' };
  await db.q(`DELETE FROM sessions WHERE staff_id = $1`, [r.id]);
  await audit('Уволен сотрудник', u.fullName, fullName);
  return { success: true, message: 'Уволен. В списках останется ещё 3 месяца — на случай жалобы' };
}

async function getAllUsers(token) {
  const u = await need(token, STAFF_ADMINS);
  if (!u.success) return u;
  const rows = await db.q(`SELECT full_name, team, role, login, position, hired_at, training_at,
                                  active, dismissed_at,
                                  (password_hash IS NOT NULL) AS has_pass
                             FROM staff WHERE ${db.STILL_VISIBLE}
                             ORDER BY active DESC, team, full_name`);
  return { success: true, users: rows.map(r => ({
    fullName: r.full_name, group: r.team, role: r.role, position: r.position || '',
    login: r.login, hasPassword: r.has_pass,
    active: r.active,
    dismissed: r.dismissed_at ? core.fmtDate(r.dismissed_at) : '',
